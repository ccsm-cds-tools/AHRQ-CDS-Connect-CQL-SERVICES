...
async function load(pathToFolder) {
  ...
  for (const folderName of readdirSync(pathToFolder)) {
    ...
    if (stats.isDirectory()) {
      ...
      let { elmJson, cdsResources, valueSetJson, prefetch, formatCards, collapseIntoOne } = await import(pathToFileURL(index));
      prefetch = prefetch ?? extractPrefetch(Object.values(elmJson));
      forApplying[folderName] = { elmJson, cdsResources, valueSetJson, prefetch, formatCards, collapseIntoOne };
    } else {
      continue;
    }
  }
